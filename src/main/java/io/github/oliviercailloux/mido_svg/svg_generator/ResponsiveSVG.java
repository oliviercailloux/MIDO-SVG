...
/**
 * This class adapts the position of objects in order to draw a responsive SVG according to the
 * user's settings.
 *
 */
public class ResponsiveSVG {
   ...
   * @param myYear is a key in mapOfGrade. 1 representing the first year (L1) and 5 the last year
   *        (M2)
   ...
   * 
   * @param list is a Collection of formations
   * @param myYear the year for which we want to find the maximum number of subjects. 1 representing
  ...
  public int calculateHeightOfSubjects(int nbOfSubjects) {
    ...
    int heightOfSubjects =
        (nbOfSubjects + 1) * height + (nbOfSubjects + 2) * DrawerSVGGen.SPACE_BETWEEN_LIGNES;
  ...
  /**
   * This method aim to calculate the space that is available to add between the formations of
   * different grade
   * 
   * @param canvasY
   * @param maxSubjects
   ...
   */
  public int calculateAdditionalSpaceY(int canvasY, Collection<Integer> maxSubjects,
      int totalCptY) {
    ...
    int additionalSpace = (int) (((canvasY - totalHeightOfSubjects - (canvasY * 0.15)
        - formationBlock * (totalCptY - 1)) / totalCptY) + formationBlock);

    return additionalSpace;
  }

  /**
   * This method aim to calculate the space that is available to add between the formations of a
   * same year when drawing.
   * 
   ...
   * @return an int corresponding to the space in pixels to add between the formations of a same
   *         year
  ...
  /**
   * Determine the position of each Formation in the Canvas according to the number of formations
   * and their level.
   *
  ...
  public void defineObjectsPosition(Collection<Formation> initialList, int canvasX, int canvasY,
    ...
    /*
     * We have to define the initial shift. So we must count the total number formations in "list"
     * then calculate Y offset depending on which formation are chosen (Only L1 or all ?). Finally
     * we calculate X offset
     */
    int offsetY = 0;
    ...
    /*
     * O<=totalCptY<=5 corresponds to the DOM Tree height totalCptY = 5 means we have to draw the
     * formations from level "L1" to "M2"
     */
    int totalCptY = 0;

    /*
     * We assume that the indexes correspond to the levels ordered from "L1" to "M2" For the moment
     * we just initialize the tab with zeros Afterwards, depending on which levels the user choose
     * to draw, each level will have a certain height in the tree
     */
    int cptY[] = {0, 0, 0, 0, 0};
     ...
     * We fill cptY
     * 
     * For example if nbL1==0 and nbL2 == 0 then cpt[2] == 1 which means that the level "L3" has the
     * height 1 in the tree which means "L3" is the root of the tree.
     * 
     */
    int tempCpt = 0;
     ...
     * 
     * For each level: - if the subjects are going to be drawn, the Y offset is calculated depending
     * on the additionalSpace calculated before. - if the subjects are not going to be drawn, we
     * divide the space depending on the number of level drawn
     */
    for (int i = 1; i < 6; i++) {
      if (numberOfGrade.get(i) != 0) {
        ...
        if (hiddenSubjects) {
          ...
          if (levelActual == 1) {
            ...
            levelActual++;
            spaceTaken +=
                offsetY + calculateHeightOfSubjects(countMaxSubjects(list, i)) + additionalSpace;
          } else {
            offsetY = spaceTaken;
            levelActual++;
