...
Objectif principal : Modifier la classe `svg_generator.ResponsiveSVG` et ajouter une étape de contrôle avant la génération du graphe (évitant les superpositions des éléments).
...
image::https://github.com/marcellinodour/MIDO-SVG/blob/master/docs/svgmargins.jpg[Schema Margin, 602, 416,role="right"]
...
Pour aller plus loin : Si tout est terminé, créer une nouvelle méthode (suggestion : `isOverlap`) dans la classe `svg_generator.ResponsiveSVG` qui permet, avant chaque paramétrage d'un point d'une formation, le contrôle du chevauchement (exemple s'assurer que la position x de la formation d'après `new_x > maxNumberOfCourse (de la formation précédente) && new_x < setting.length`)
