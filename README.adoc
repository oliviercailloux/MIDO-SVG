...
* Les longueurs et largeurs des listes de cours correspondent à un nombre de pixels. Pour déterminer précisément ce nombre de pixels il faudrait également récupérer des éléments graphiques représentant la taille de la police, l’espace entre les lignes, les marges… Une solution intermédiaire consisterait à utiliser des variables de classe pour stocker ces éléments et les utiliser dans le calcul.
* Vous pouvez partie de la branche dev-BDD2 ou du master
